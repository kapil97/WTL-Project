...
{
  var error="";
  if(!(/[^a-zA-Z]/(document.reg.name.value) ) )
  {
      
      error=error+"Name cannot contain numbers or special characters"
      document.reg.name.value=""
  }
  if(document.reg.pass.value!=document.reg.repass.value)
  {
    error=error+"Re-entered password is wrong"
...
?>
    <?php
     if($flag == 1)
    {
		$qinsert = "insert userinfo VALUES(DEFAULT,'$signup_name',$signup_username','$signup_emailid','$signup_pass')";
